  ...
  return (
    <div className="flex-1 flex gap-7 max-w-[1400px] overflow-y-scroll h-screen px-1">
      <div className="flex-1 flex flex-col gap-[1rem] bg-white">
        ...
        <div className="flex w-full h-full justify-center items-center"> {/* when no job exist */}
          {/* <JobListCard />
          <JobListCard />
          <JobListCard />
          <JobListCard />
          ...
          <div className='flex flex-col items-center gap-4'>
            ...
            <div className='flex flex-col gap-3 items-center'>
              <div className='flex gap-1 flex-col items-center'>
                <p className='text-[#404040] font-bold text-[1.25rem]'>No job openings available</p>
                <p className='text-[#404040] text-[1rem]'>Create a job opening now and start the candidate process.</p>
              </div>
              <div>
                <button onClick={() => setOpenDialog(!openDialog)} className='px-[1rem] py-[6px] rounded-[8px] text-[#404040] text-[1rem] font-bold bg-[#FBC037] hover:bg-[#f5b621] cursor-pointer  shadow'>
                  Create a new job
                </button>
              </div>
            </div>
      ...
      <div className="w-[300px] hidden lg:block">
        ...
        >
          ...
          <button onClick={() => setOpenDialog(!openDialog)} className="bg-[#01959F] group/button-add cursor-pointer overflow-hidden relative z-30 text-white text-[1rem] font-bold py-1.5 px-4 rounded-[6px] w-full">
      ...
      <Tooltip>
        <TooltipTrigger onClick={() => setOpenDialog(!openDialog)} className="fixed bottom-5 right-5 w-10 h-10 rounded-full flex items-center justify-center shadow bg-(--secondary-color) text-white cursor-pointer hover:bg-[#037881] lg:hidden">
          <Plus size={16} />
        </TooltipTrigger>
        <TooltipContent>
          <p>Create new job</p>
